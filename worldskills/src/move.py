...
class LineController:
	def __init__(self):
		...
		with open("/home/ubuntu/ros_package_rp_worldskills/src/worldskills/src/calibration.json", "r") as read_file:
	...
	def command_cb(self,data):
		self.start_move = data.data[0]
		if self.target_marker != data.data[1]:
			self.target_marker = data.data[1]
	...
	def main_moving(self):
		if self.start_move == 1:
			self.count = 0
			if self.target_point == False:
				if self.state == 5:
					if(self.from_park_left() == False):
						return
				if self.state == 6:
					if(self.from_park_right() == False):
						return
				if self.current_zone == 0:
					self.go_on_line_left()
					self.analize_line()
				elif self.current_zone == 1:
					self.go_on_line_front()
					self.analize_line()			
				elif self.current_zone == 2:
					self.go_on_line_right()
					self.analize_line()			
				elif self.current_zone == 3:
					self.go_on_line_back()
					self.analize_line()			
				self.start_park = True
				self.integral = 0
				...
				if self.target_marker < 3:
					if(self.start_park == True):
						if(self.to_park_left() == True):
							self.start_park = False
					else:
						self.parallel_parking_left()
				else:
					if(self.start_park == True):
						if(self.to_park_right() == True):
							self.start_park = False
					else:
	...
	def go_on_line_right(self):
		...
		self.speed_publisher(up_x, self.speed, -up_z)
		self.state = 3
	def analize_line(self):
		front = self.goal_light[0][1] + self.goal_light[0][2] + self.goal_light[0][3] + self.goal_light[0][4] + self.goal_light[0][5] + self.goal_light[0][0] + self.goal_light[0][6] 
		left = self.goal_light[1][1] + self.goal_light[1][2] + self.goal_light[1][3] + self.goal_light[1][4] + self.goal_light[1][5]  + self.goal_light[1][0] + self.goal_light[1][6]
		right = self.goal_light[2][1] + self.goal_light[2][2] + self.goal_light[2][3] + self.goal_light[2][4] + self.goal_light[2][5] + self.goal_light[2][0] + self.goal_light[2][6]
		back = self.goal_light[3][1] + self.goal_light[3][2] + self.goal_light[3][3] + self.goal_light[3][4] + self.goal_light[3][5] + self.goal_light[3][0] + self.goal_light[3][6]
		if front > 0 and self.current_zone == 0:
			if float(time() - self.previous_scan_time) > 2:
				self.current_zone = 1
				self.previous_scan_time = time()
		if right > 0 and front < 1 and self.current_zone == 1:
			if float(time() - self.previous_scan_time) > 2:
				self.current_zone = 2
				self.previous_scan_time = time()

		if back > 0 and right < 1 and self.current_zone == 2:
			if float(time() - self.previous_scan_time) > 2:
				self.current_zone = 3
				self.previous_scan_time = time()

		if left > 0 and back < 1 and self.current_zone == 3:
			if float(time() - self.previous_scan_time) > 2:
				self.current_zone = 0
				self.previous_scan_time = time()
